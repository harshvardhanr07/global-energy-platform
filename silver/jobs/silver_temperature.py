"""
silver_temperature.py
Reads bronze api/temperature from S3, applies cleaning/DQ/dedup,
writes silver api/temperature back to S3.
...
TABLE_KEY   = "api/temperature"
TEMP_CHECKS = [
    ("avg_outside_temp",   -50.0, 60.0),
    ("degree_day_cooling",   0.0, None),
    ("degree_day_heating",   0.0, None),
    ("reference_temp",     -50.0, 60.0),
]


class SilverTemperatureJob:

    def __init__(self, spark: SparkSession, pg_conn, bronze_root: str, silver_root: str):
        self.spark       = spark
        self.pg_conn     = pg_conn
        self.bronze_path = f"{bronze_root}/api/temperature"
        self.silver_path = f"{silver_root}/api/temperature"
    ...
    def run(self):
        ...
        try:
            ...
            checks = [
                ...
                *[
                    (lambda col, lo, hi: lambda d: assert_range(d, col, lo, hi, TABLE_KEY))(c, mn, mx)
                    for c, mn, mx in TEMP_CHECKS
                ],
