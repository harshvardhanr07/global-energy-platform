"""
silver_site_status_history.py
Reads bronze db/site_status_history from S3, applies cleaning/DQ/dedup,
writes silver db/site_status_history back to S3.
Incremental load via watermark on started_on column.
"""
...
TABLE_KEY = "db/site_status_history"


class SilverSiteStatusHistoryJob:

    def __init__(self, spark: SparkSession, pg_conn, bronze_root: str, silver_root: str):
        self.spark       = spark
        self.pg_conn     = pg_conn
        self.bronze_path = f"{bronze_root}/db/site_status_history"
        self.silver_path = f"{silver_root}/db/site_status_history"
    ...
    def run(self):
        ...
        try:
            ...
            if watermark_ts:
                ...
                df = df.filter(F.col("started_on") > watermark_ts)
            else:
            ...
            # 3. normalize strings
            df = normalize_strings(df, cols=["site_id", "event_type", "new_site_id"])
            ...
            # 5. DQ checks
            checks = [
                lambda d: assert_no_nulls(d, ["site_id", "started_on"], TABLE_KEY),
                lambda d: assert_unique(d, ["site_id", "started_on", "event_type"], TABLE_KEY),
            ]
            ...
            df = deduplicate(
                df,
                keys=["site_id", "started_on", "event_type"],
            ...
            # 9. update watermark
            max_date = df.agg(F.max("started_on")).collect()[0][0]
