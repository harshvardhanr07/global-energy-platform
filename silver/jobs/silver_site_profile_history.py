"""
silver_site_profile_history.py
Reads bronze db/site_profile_history from S3, applies cleaning/DQ/dedup,
writes silver db/site_profile_history back to S3.
Incremental load via watermark on changed_on column.
"""
...
TABLE_KEY = "db/site_profile_history"


class SilverSiteProfileHistoryJob:

    def __init__(self, spark: SparkSession, pg_conn, bronze_root: str, silver_root: str):
        self.spark       = spark
        self.pg_conn     = pg_conn
        self.bronze_path = f"{bronze_root}/db/site_profile_history"
        self.silver_path = f"{silver_root}/db/site_profile_history"
    ...
    def run(self):
        ...
        try:
            ...
            if watermark_ts:
                ...
                df = df.filter(F.col("changed_on") > watermark_ts)
            else:
            ...
            # 3. normalize strings
            df = normalize_strings(df, cols=["site_id", "change_field"])
            ...
            # 5. DQ checks
            checks = [
                lambda d: assert_no_nulls(d, ["site_id", "changed_on"], TABLE_KEY),
                lambda d: assert_unique(d, ["site_id", "change_field", "changed_on"], TABLE_KEY),
            ]
            ...
            df = deduplicate(
                df,
                keys=["site_id", "change_field", "changed_on"],
            ...
            # 9. update watermark
            max_ts = df.agg(F.max("changed_on")).collect()[0][0]
