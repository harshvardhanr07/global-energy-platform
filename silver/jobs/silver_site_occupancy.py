"""
silver_site_occupancy.py
Reads bronze db/site_occupancy from S3, applies cleaning/DQ/dedup,
derives occupancy_pct, writes silver db/site_occupancy back to S3.
Incremental load via watermark on date column.
"""
...
TABLE_KEY = "db/site_occupancy"


class SilverSiteOccupancyJob:

    def __init__(self, spark: SparkSession, pg_conn, bronze_root: str, silver_root: str):
        self.spark       = spark
        self.pg_conn     = pg_conn
        self.bronze_path = f"{bronze_root}/db/site_occupancy"
        self.silver_path = f"{silver_root}/db/site_occupancy"
    ...
    def run(self):
        ...
        try:
            ...
            if watermark_ts:
                ...
                df = df.filter(F.col("date") > watermark_ts)
            else:
            ...
            # 5. derive occupancy_pct
            df = df.withColumn(
                "occupancy_pct",
                F.when(
                    F.col("site_capacity") > 0,
                    (F.col("occupancy") / F.col("site_capacity") * 100).cast("double"),
                ).otherwise(F.lit(None).cast("double")),
            )

            # 6. DQ checks
            checks = [
                lambda d: assert_no_nulls(d, ["site_id", "date"], TABLE_KEY),
                lambda d: assert_range(d, "occupancy", 0, None, TABLE_KEY),
                lambda d: assert_unique(d, ["site_id", "date"], TABLE_KEY),
            ]
            df, dq_result = run_checks(df, TABLE_KEY, checks)

            # 7. deduplicate
            df = deduplicate(df, keys=["site_id", "date"], order_col="_ingested_at")
