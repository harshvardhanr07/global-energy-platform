"""
silver_site_profile.py
Reads bronze db/site_profile from S3, applies cleaning/DQ/dedup,
writes silver db/site_profile back to S3.
Full overwrite every run — keep most recent profile per site.
"""
...
TABLE_KEY = "db/site_profile"


class SilverSiteProfileJob:

    def __init__(self, spark: SparkSession, pg_conn, bronze_root: str, silver_root: str):
        self.spark       = spark
        self.pg_conn     = pg_conn
        self.bronze_path = f"{bronze_root}/db/site_profile"
        self.silver_path = f"{silver_root}/db/site_profile"
    ...
    def run(self):
        ...
        try:
            ...
            # 5. deduplicate — keep most recent profile per site
            df = deduplicate(df, keys=["site_id"], order_col="last_updated")
